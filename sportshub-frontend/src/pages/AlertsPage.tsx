...
export function AlertsPage() {
  ...
  return <section className="workspace-page narrow"><div className="page-intro"><span className="eyebrow">Stay in the moment</span><h1>Match alerts</h1><p>These settings apply globally to your account—not separately to each team.</p></div><div className="panel settings-panel">{error && <div className="error-message" role="alert">{error}</div>}{saved && <div className="success-message" role="status">Preferences saved.</div>}{!preferences ? <div className="empty-state">Loading your preferences...</div> : Object.entries(labels).map(([key, [title, description]]) => {
